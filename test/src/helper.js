...
const path2 = {
    svgs: {
        index: path.resolve("test/assets/svgs"),
    },
    expected: {
        index: path.resolve("test/assets/expected"),
    },
    generated: {
        index: path.resolve("test/assets/generated"),
    },
};

Object.assign(path2.expected, {
    svg: {
        resize: {
            index: path.join(path2.expected.index, "svg/resize"),
        },
    },
    output: {
        index: path.join(path2.expected.index, "output"),
    },
    extend: {
...
function compare(actual, expected) {
    return new Promise((resolve, reject) => {
        looksame(actual, expected, { strict: true }, (err, data) => {
            if (err) {
                reject(err);
                return;
            }
...
var inputs = {
    ...
    data: [
        ...
        },
        {
            ...
            data: path.join(path2.svgs.index, "normal.svg"),
            description: "path to valid SVG",
        },
        {
            name: "valid-svg-string",
            data: fs.readFileSync(path.join(path2.svgs.index, "normal.svg"), "utf-8"),
            description: "valid SVG string",
        },
        {
            name: "valid-svg-in-buffer",
            data: fs.readFileSync(path.join(path2.svgs.index, "normal.svg")),
