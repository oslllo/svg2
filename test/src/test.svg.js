...
describe("svg.js", () => {
  describe("new Svg()", () => {
    describe("svg.update()", () => {
      ...
      describe("- results", () => {
        it("can update instance SVG using (SVG Element)", () => {
          ...
          var update = fs.readFileSync(path.join(path2.svgs.index, "normal-2.svg"), "utf-8");
          ...
          assert.notEqual(current, update, "current and new SVG strings are identical");
    ...
    describe("svg.resize()", () => {
      describe("- arguments", () => {
        describe("[input]", () => {
          var data = ["number", "object-with-width-and-height", "object-with-height", "valid-svg-resize-object-width"];
          var instance = Svg2(path.join(path2.svgs.index, "normal.svg"));
          var svg = instance.svg;
      ...
      describe("- results", () => {
        it("can set SVG width and height", async () => {
          ...
          assert.notDeepEqual(svg.dimensions(), dimensions, "current and new dimensions do not match before change");
          svg.resize(dimensions);
          assert.deepEqual(svg.dimensions(), dimensions, "dimensions match after change");
        });
        it("can set SVG height without changing width", () => {
          ...
          assert.notDeepEqual(svg.dimensions(), expected, "current and new dimensions do not match before change");
          svg.resize(dimensions);
          assert.deepEqual(svg.dimensions(), expected, "dimensions match after change");
        });
    ...
    describe("svg.dimensions()", () => {
      describe("- results", () => {
        it("throws when SVG does not have width and height attributes or a viewBox", () => {
          var instance = Svg2(path.join(path2.svgs.index, "without-viewbox-or-dimension-attributes.svg"));
          var svg = instance.svg;
        ...
        it(`can adjust width without changing height`, () => {
          ...
          assert.deepEqual(svg.dimensions(), current, "svg dimensions and expected do not match");
          ...
          assert.equal(scale, 2.5833333333333335, "svg did not scale correctly");
        });
        it(`can adjust height without changing width`, () => {
          ...
          assert.deepEqual(svg.dimensions(), { width: 24, height: 48 }, "svg dimensions did not update correctly");
          var scale = instance.output.resize.scale;
        ...
        it(`can adjust height with AUTO width (even)`, () => {
          ...
          assert.deepEqual(svg.dimensions(), { width: 144, height: 144 }, "svg dimensions did not update correctly");
          var scale = instance.output.resize.scale;
          assert.equal(scale, 6, "svg did not scale correctly");
        });
        it(`can adjust width with AUTO height (uneven) (positive change)`, () => {
          var instance = Svg2(path.join(path2.svgs.index, "with-uneven-px-dimensions.svg"));
          var svg = instance.svg;
        ...
        it(`can adjust width with AUTO height (uneven) (negetive change)`, () => {
          ...
          assert.deepEqual(svg.dimensions(), { width: 190, height: 134.44 }, "svg dimensions did not update correctly");
        });
        ...
        it(`can scale height and width (even)`, () => {
          ...
          assert.deepEqual(svg.dimensions(), current, "svg dimensions and expected do not match");
          svg.resize(2);
          assert.deepEqual(svg.dimensions(), { width: 48, height: 48 }, "svg dimensions did not update correctly");
        });
        it(`can scale height and width (uneven)`, () => {
          ...
          assert.deepEqual(svg.dimensions(), current, "svg dimensions and expected do not match");
          svg.resize(2);
          assert.deepEqual(svg.dimensions(), { width: 38, height: 26.888 }, "svg dimensions did not update correctly");
        });
      });
    });
    describe("svg.html()", () => {
