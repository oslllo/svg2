  ...
  describe("png()", () => {
    describe("- results", () => {
      it("can set instance output format to (png)", () => {
        var svg = Svg2(path.join(path2.svgs.index, "normal.svg"));
      ...
      it("can export (png)", async () => {
        var name = "svg-output.png";
        var destination = path.join(path2.generated.index, name);
        fs.removeSync(destination);
        assert.isFalse(fs.existsSync(destination), "failed to removed existing image");
        var svg = Svg2(path.join(path2.svgs.index, "normal.svg"));
    ...
    describe("- arguments", () => {
      describe("[input]", () => {
        it("can return svg instance element with no input parameter", () => {
          ...
          assert.equal(element.constructor.name, "SVGSVGElement", "element is not an instance of SVGSVGElement");
          ...
          assert.equal(current, raw, "raw svg html does not match element html");
        });
        it("can return svg element provided in input parameter", () => {
          ...
          assert.isTrue(is.svg(raw), `raw SVG is not a valid svg, ${raw}`);
          assert.equal(outerHTML, raw, "raw svg html does not match element html");
        });
      });
    });
  });
  describe("toUrl()", () => {
    ...
    describe("- results", () => {
      ...
      it("can output a valid base64 string (callback)", (done) => {
        Svg2(path.join(path2.svgs.index, "normal.svg")).toUri({ base64: true }, (err, base64) => {
          assert.equal(is.base64(base64), true);
    ...
    describe("- results", () => {
      ...
      it(`can convert to png image and return buffer image that looks correct`, (done) => {
        ...
        svg.png().toBuffer((err, buffer) => {
          error = err ? err : error;
          assert.isTrue(Buffer.isBuffer(buffer), "did not return buffer");
          looksame(buffer, path.join(path2.generated.index, "svg-output.png"), { strict: true }, (err, data) => {
            error = err ? err : error;
  ...
  describe("toFile()", () => {
    describe("- arguments", () => {
      describe("[destination]", () => {
        var data = inputs.data
          .filter((input) => {
            if (typeof input.data === "string" || input.name.search("string") !== -1) {
              return true;
            }
            return false;
          })
        ...
        inputs.invalid(data).forEach((input) => {
          it(`throws with invalid destination parameter (${input.description}) (promise)`, () => {
            var svg = Svg2(path.join(path2.svgs.index, "normal.svg"));
            return expect(svg.png().toFile(input.data)).to.be.rejectedWith(TypeError);
          });
        });
        inputs.invalid(data).forEach((input) => {
      ...
      describe("[callback]", () => {
        ...
        inputs.invalid(data).forEach((input) => {
          it(`throws with invalid callback parameter (${input.description})`, () => {
            ...
            var name = "svg.png";
            var destination = path.join(path2.generated.index, name);
            assert.throws(() => svg.png().toFile(destination, input.data), TypeError);
          });
        });
        ...
        inputs.valid(data).forEach((input) => {
          it(`does not throw with valid callback parameter (${input.description})`, () => {
            var svg = Svg2(path.join(path2.svgs.index, "normal.svg"));
            var name = "svg-callback.png";
            var destination = path.join(path2.generated.index, name);
            assert.doesNotThrow(() => svg.png().toFile(destination, input.data), Error);
          });
        });
      });
    });
    describe("- results", () => {
      ...
      it(`can convert to png image and save to path (promise)`, async () => {
        var name = "svg.png";
        var destination = path.join(path2.generated.index, name);
        var svg = Svg2(path.join(path2.svgs.index, "normal-2.svg"));
        await svg.png().toFile(destination);
